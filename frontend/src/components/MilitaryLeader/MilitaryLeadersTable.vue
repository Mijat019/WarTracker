<template>
    <div>
        <v-data-table
                :headers="headers"
                :items="militaryLeaders"
                class="elevation-1"
                sort-by="calories"
        >
            <template v-slot:top>
                <v-toolbar color="white" flat>
                    <v-toolbar-title>Military Leaders</v-toolbar-title>
                    <v-divider
                            class="mx-4"
                            inset
                            vertical
                    ></v-divider>
                    <v-spacer></v-spacer>
                    <v-btn @click="addItem" class="mb-2" color="primary" dark>New Military Leader</v-btn>

                </v-toolbar>
            </template>

            <template v-slot:item.update="{ item }">
                <v-icon
                        @click="editItem(item)"
                        color="amber darken-2"
                >
                    mdi-pencil
                </v-icon>
            </template>
            <template v-slot:item.remove="{ item }">
                <v-icon
                        @click="deleteItem(item)"
                        color="red"
                >
                    mdi-delete
                </v-icon>
            </template>
        </v-data-table>
        <military-leader-modify
                mode="add"
                v-model="addDialog"/>
        <military-leader-modify
                :edit-military-leader="editMilitaryLeader"
                mode="update"
                v-model="editDialog"/>
    </div>
</template>

<script>
    import {mapActions, mapState} from "vuex";
    import MilitaryLeaderModify from "./MilitaryLeaderModify";

    export default {
        name: "MilitaryLeadersTable",
        components: {MilitaryLeaderModify},
        data: () => ({
            editDialog: false,
            addDialog: false,
            editMilitaryLeader: null,
            dialog: false,
            headers: [
                {text: 'Id', value: 'id'},
                {text: 'Name', value: 'firstName'},
                {text: 'Last Name', value: 'lastName'},
                {text: 'Dynasty Name', value: 'dynastyName'},
                {text: 'Military Rank', value: 'militaryRank'},
                {text: 'Title', value: 'title'},
                {text: 'Update/Details', value: 'update', sortable: false, align: 'center'},
                {text: 'Remove', value: 'remove', sortable: false, align: 'center'}
            ],
        }),
        computed: {
            ...mapState('militaryLeader', ['militaryLeaders']),
        },

        created() {
            this.initialize()
        },

        methods: {
            ...mapActions("militaryLeader", ["getMilitaryLeaders"]),
            ...mapActions("militaryLeader", ["deleteMilitaryLeader"]),
            initialize() {
                this.getMilitaryLeaders();
            },

            editItem(militaryLeader) {
                this.editMilitaryLeader = militaryLeader;
                this.editDialog = true;
            },
            addItem() {
                console.log(this.militaryLeaders);
                this.addDialog = true;
            },
            deleteItem(item) {
                let msg = "Are you sure you want to remove "+item.firstName + " " + item.lastName +  "?"
                confirm(msg) && this.deleteMilitaryLeader(item)
            },
        },
    }
</script>

<style scoped>

</style>