    ...
    export default {
        ...
        watch: {
            ...
            '$store.state.tutorial.battlePositions.length'(newLength, oldLength) {
                if(oldLength === 0)
                    this.placeMarkers(this.$store.state.tutorial.battlePositions, battleType);
                else if (newLength === 0 && !this.$store.state.tutorial.recentlyDeleted) {
                    return;
                }
                else if (newLength > oldLength){
                    let added = this.$store.state.tutorial.newlyAdded;
                    this.placeMarkers([].concat(added), battleType);
                    this.$store.commit('positions/resetAdded');
                } else if (oldLength > newLength) {
                    let deleted = this.$store.state.tutorial.recentlyDeleted;
                    this.removeMarker(deleted, battleType);
                    this.$store.commit('positions/resetDeleted');
                }
            },
            '$store.state.tutorial.militaryLeaderPositions.length'(newLength, oldLength) {
                if(oldLength === 0)
                    this.placeMarkers(this.$store.state.tutorial.militaryLeaderPositions, militaryLeaderType);
                else if (newLength === 0 && !this.$store.state.tutorial.recentlyDeleted) {
                    return;
                }
                else if (newLength > oldLength){
            ...
            '$store.state.tutorial.militaryLeaderBattles.length'(newLength, oldLength) {
                if(oldLength === 0)
                    this.placeLines(this.$store.state.tutorial.militaryLeaderBattles);
                else if (newLength === 0 && !this.$store.state.tutorial.recentlyDeleted) {
                    this.removeLines([])
                }
                else if (newLength > oldLength) {
        ...
        methods: {
                ...
                if (this.disconnectLine.militaryLeader && this.disconnectLine.battle) {
                    ...
                    if(!edge) {
                        ...
                        if(this.ongoingTutorial) {
                            this.$store.commit('map/setLineRemoving', false);
                            this.$store.commit('tutorial/increaseStep');
                            this.$store.dispatch('tutorial/removeMilitaryLeaderBattle');
                        } else {
            ...
            connectTwo(event, position, type) {
                ...
                if (this.line.militaryLeader && this.line.battle) {
                    ...
                    if(this.ongoingTutorial) {
                        this.$store.commit('tutorial/increaseStep');
                        this.$store.dispatch('tutorial/addMilitaryLeaderBattle', connection);
                    } else {
            ...
            async dropped(event) {
                ...
                console.log(event.data.type.label);
                ...
                if(event.data.type.label === militaryLeaderType.label) {
                    ...
                    if(this.ongoingTutorial) {
                        this.$store.commit('tutorial/increaseStep');
                        await this.$store.dispatch('tutorial/addMilitaryLeaderPosition', position);
                    } else {
