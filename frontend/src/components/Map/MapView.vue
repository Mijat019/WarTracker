<template>
  <v-container fluid class="ma-0 pa-0" style="min-height:100vh;">
    <drop @drop="dropped">
      ...
      <div :id="mapId" style="min-width: 100vh; min-height: 100vh; z-index: 0;">
        <position-popup />
      </div>
    </drop>
  </v-container>
...
export default {
  ...
  watch: {
    ...
    updatedMilitaryLeader(newVal) {
      if (!newVal) return;
      let marker = this.placed[
        militaryLeaderType.label + newVal.militaryLeader.id
      ].marker;
      this.markerCluster.removeLayer(marker);
      marker.remove();
      let oldEdges = this.placed[
        militaryLeaderType.label + newVal.militaryLeader.id
      ].edges;
    ...
    "militaryLeaderPositions.length"(newLength, oldLength) {
      ...
      if (oldLength === 0)
        ...
        newLength === 0 &&
        !this.$store.state.positions.recentlyDeleted
      ) {
        return;
      } else if (newLength > oldLength) {
        // dodat military leader
    ...
    "battlePositions.length"(newLength, oldLength) {
      // Proverava velicinu pozicija i na taj nacin se abdejtuje
      if (oldLength === 0) this.placeMarkers(this.battlePositions, battleType);
      else if (
        newLength === 0 &&
        !this.$store.state.positions.recentlyDeleted
      ) {
    ...
    "militaryLeaderBattles.length"(newLength, oldLength) {
      if (oldLength === 0) this.placeLines(this.militaryLeaderBattles);
      else if (
        newLength === 0 &&
        !this.$store.state.militaryLeaderBattles.recentlyDeleted
      ) {
        return;
      } else if (newLength > oldLength) {
  ...
  methods: {
    ...
    disconnectTwo(event, position, type) {
      ...
      if (this.disconnectLine.militaryLeader && this.disconnectLine.battle) {
        let { edges } = this.placed[
          militaryLeaderType.label +
            this.disconnectLine.militaryLeader.militaryLeader.id
        ];
        let edge = edges.find(
          edge => edge.to.id === this.disconnectLine.battle.id
        );
        if (!edge) {
          this.$store.commit("snackbar/openSnackbar", {
            text: `There isn't any connection between selected entities`,
            color: "error"
          });
        } else {
    ...
    removeLine(militaryLeaderBattle) {
      let leaderPlaced = this.placed[
        militaryLeaderType.label + militaryLeaderBattle.militaryLeaderId
      ];
      ...
      let battlePlaced = this.placed[
        battleType.label + militaryLeaderBattle.battleId
      ];
    ...
    placeLines(militaryLeaderBattles) {
      for (let militaryLeaderBattle of militaryLeaderBattles) {
        let mlPlaced = this.placed[
          militaryLeaderType.label + militaryLeaderBattle.militaryLeaderId
        ].element;
        let baPlaced = this.placed[
          battleType.label + militaryLeaderBattle.battleId
        ].element;
    ...
    placeLine(id, militaryLeaderPosition, battlePosition) {
      ...
      this.placed[
        militaryLeaderType.label + militaryLeaderPosition.militaryLeader.id
      ].edges.push({
        id,
        line,
        to: battlePosition
    ...
    removeMarker(position, type) {
      let id =
        type.label === battleType.label
          ? position.battle.id
          : position.militaryLeader.id;
    ...
    async markerMoved(event, position, type) {
      ...
      if (type.label === battleType.label) {
        ...
        if (!address) {
          ...
          this.$store.commit("snackbar/openSnackbar", {
            text: "A military leader cannot be placed on international waters.",
            color: "error"
          });
          return;
        }
    ...
    async checkDrop(data) {
      try {
        const { data: existing } = await Vue.$axios.get(
          `${data.type.label}MapPosition/check/${data.id}/mapNot/${this.mapObj.id}`
        );
        return existing;
      } catch (err) {
        return null;
