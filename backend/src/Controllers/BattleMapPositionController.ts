...
class BattleMapPositionController {
    ...
    public async checkPlacement(req: Request, res: Response) {
        try {
            ...
            const exists = await battleMapPositionService.exists(
                battleId,
                mapId
            );
            if (exists) {
    ...
    public async getAndFilter(req: any, res: any) {
        try {
            const { mapId } = req.params;
            const { filter, search } = req.body;
            const battlePositions = await battleMapPositionService.getAndFilter(
                mapId,
                filter,
                search
            );
            res.send(battlePositions);
        } catch (error) {
            res.status(400).send(error.message);
        }
    }
